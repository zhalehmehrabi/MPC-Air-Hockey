...
class MPC():
    ...
    def building_optimizer(self):
        ...
        # eex, eey, eez = ca.MX.sym('eex'), ca.MX.sym('eey'), ca.MX.sym('eez')
        # -- conctenated vector
        ...
        _u = ca.MX.sym('_u', self.n_controls)
        _x = ca.MX.sym('_x', self.n_states)

        # x_dot = myJacobian(_u[:3], _u[3:])
        #
        x_dot = _x + _u[:3]

        self.f = ca.Function('f', [_u, _x], [x_dot], ['_u', '_x'], ['x_dot'])
        ...
        Delta_s = ca.MX.sym("Delta_s", self.n_states)
        Delta_u = ca.MX.sym("Delta_u", self.n_controls)
        ...
        self.P = ca.MX.sym("P", self.n_states+(self.n_states+3)*self.N+self.n_states)
        ...
        X_next = next_state_creator(self.X[:,0], self.U)
...
class MyJacobian(casadi.Callback):
    ...
    def eval(self, arg):
        pos = arg[0]
        vel = arg[1]
        f = jacobian(self.mjmodel, self.mjdata, x) @ vel
        return [f]
