...
R = zeros(6,6); R(1,1) = 0.1; R(2,2) = 0.1;R(3,3) = 0.1;
                R(4,4) = 0.1; R(5,5) = 0.1;R(6,6) = 0.1;% weighing matrices (controls)
...
%-------------------------------------------
t0 = 0;
x0 = [0 ; 0 ; 0.1];    % initial condition.
xs = [-0.5 ; 0 ; 0.1]; % Reference posture.
...
while(norm((x0-xs),2) > 1e-2 && mpciter < sim_tim / T)
    ...
    [t0, x0, u0] = shift(T, t0, x0, u,n_controls);
    xx(:,mpciter+2) = x0;
